...
public class AtualizacaoAtualizarCadastroFuncionario extends JPanel {
	 ...
	 * Create the panel.
	 */
	public AtualizacaoAtualizarCadastroFuncionario(Funcionarios funcionario) {
		...
		campoIdade.setText(String.valueOf(funcionario.getIdade()));
		...
		String[] funcoes = { "", "Atendente", "Administrador" };
		...
		if (funcionario.getTipoFuncionario().charAt(0) == 'A' && funcionario.getTipoFuncionario().charAt(1) == 'T') {
			selectFuncao.setSelectedItem("Atendente");
		} else {
			selectFuncao.setSelectedItem("Administrador");
		}

		/*
		 * JLabel textoRmv = new JLabel("RMV:"); textoRmv.setFont(new Font("Tahoma",
		 * Font.PLAIN, 16)); textoRmv.setBounds(130, 209, 62, 20); panel.add(textoRmv);
		 * 
		 * campoRmv = new JFormattedTextField(mfCrmv); campoRmv.setFont(new
		 * Font("Dialog", Font.PLAIN, 14)); campoRmv.setBounds(186, 207, 120, 25);
		 * panel.add(campoRmv); campoRmv.setColumns(10); campoRmv.setEditable(false);
		 */
		JButton botaoCancelar = new JButton("Cancelar");
		botaoCancelar.setForeground(new Color(255, 255, 255));
		botaoCancelar.setBackground(new Color(159, 80, 0));
		botaoCancelar.setBounds(190, 447, 120, 21);
		...
		if (funcionario.getSexo().charAt(0) == 'M') {
		...
		campoEstado.setText(funcionario.getEstado());
		...
		campoNumeroCasa.setText(String.valueOf(funcionario.getNumeroCasa()));
		...
		botaoConfirmar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (confirmacao == JOptionPane.YES_OPTION) {
					BDServices.atualizarFuncionario(funcionario.getId(), nome, idade, telefone, cpf, tipoFuncionario,
							sexo, cep, numeroCasa, rua, bairro, estado, cidade);
					Funcionarios funcionarioAtualizado = BDServices.consultarFuncionario(funcionario.getId());
					JFrame f = (JFrame) SwingUtilities.getAncestorOfClass(JFrame.class, panel);
					f.setContentPane(new FuncionariosDadosFuncionario(funcionarioAtualizado));
					f.revalidate();
				}
		 ...
		 * (selectFuncao.getSelectedItem().equals("Médico Veterinário")) {
		 * campoRmv.setEditable(true); } else { campoRmv.setEditable(false);
		 * campoRmv.setText(""); } } }; selectFuncao.addActionListener(verificarFuncao);
		 */
	...
	public AtualizacaoAtualizarCadastroFuncionario(MedicosVeterinarios funcionario) {
		setLayout(null);
		MaskFormatter mfIdade = null;
		MaskFormatter mfCpf = null;
		MaskFormatter mfCrmv = null;
		...
		JLabel atualizacaoFuncionario = new JLabel("Atualização funcionário");
		atualizacaoFuncionario.setHorizontalAlignment(SwingConstants.CENTER);
		atualizacaoFuncionario.setFont(new Font("Tahoma", Font.PLAIN, 20));
		atualizacaoFuncionario.setBounds(216, 10, 253, 25);
		panel.add(atualizacaoFuncionario);
		...
		botaoConfirmar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int confirmacao = JOptionPane.showConfirmDialog(null, "Deseja confirmar a atualização dos dados?",
						"Confirmar Atualização", JOptionPane.YES_NO_OPTION);
				if (confirmacao == JOptionPane.YES_OPTION) {
					BDServices.atualizarMedico(funcionario.getId(), nome, idade, telefone, cpf, sexo, cep, numeroCasa,
							rua, bairro, estado, cidade, rmv);
					MedicosVeterinarios funcionarioAtualizado = BDServices.consultarMedico(funcionario.getId());
					JFrame f = (JFrame) SwingUtilities.getAncestorOfClass(JFrame.class, panel);
					f.setContentPane(new FuncionariosDadosFuncionario(funcionarioAtualizado));
					f.revalidate();
				}
